...
theorem I3Bound {SmoothingF : ℝ → ℝ}
  ...
  have abs_ne_zero_of_mem_uIcc (x) (hx : x ∈ uIcc (-T) (-3)) : |x| ≠ 0 := by
    rw [abs_ne_zero]
    rintro rfl
  ...
  have int_normf_le_int_g: ∫ (t : ℝ) in (-T)..(-3), ‖f ↑t‖
    ...
    · apply intervalIntegral.integral_mono_on_of_le_Ioo Tgt3'.le
      · exact h_int
